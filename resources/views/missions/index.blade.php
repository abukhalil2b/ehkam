...
<nav class="bg-white border-b shadow-sm">
 ...
 class="bg-indigo-600 text-white px-6 py-2 rounded-xl">+ مهمة</button>
...
<div class="flex items-center gap-2 text-xs text-slate-500 mb-4">
    ...
    <div class="relative group">
        <div class="w-10 h-10 rounded-full border-2 border-white flex items-center justify-center font-bold shadow-sm transition-transform group-hover:scale-110 z-0 group-hover:z-10
            {{ $m->role === 'leader' ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-700' }}">
            {{ mb_substr($m->user->name, 0, 1) }}
            
            @if($m->role === 'leader')
                <div class="absolute -top-1 -right-1 bg-amber-400 w-3 h-3 rounded-full border border-white"></div>
            @endif
        </div>

        <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block
                    bg-slate-900 text-white text-[10px] rounded-lg px-2 py-1.5 whitespace-nowrap shadow-xl z-50">
            <div class="font-bold {{ $m->role === 'leader' ? 'text-amber-400' : '' }}">
                {{ $m->user->name }} @if($m->role === 'leader') (القائد) @endif
            </div>
    ...
    <a href="{{ route('missions.task.show',$mission->id) }}"
       class="block text-center bg-slate-900 text-white py-3 rounded-xl font-bold">
        فتح الكانبان
    </a>
...
<div x-show="open" x-cloak class="fixed inset-0 z-50 flex justify-end">
    ...
    <div class="relative w-full max-w-lg bg-white shadow-2xl h-full flex flex-col transform transition-transform"
        ...
        <form method="POST" :action="mode === 'create' ? '{{ route('mission.store') }}' : '{{ route('mission.update') }}'" class="flex-1 overflow-y-auto">
            @csrf
            ...
            <template x-for="(perm, id) in permissions" :key="'perm-'+id">
                <div>
                    <input type="hidden" :name="`permissions[${id}][can_create_tasks]`" :value="perm.can_create_tasks">
                    <input type="hidden" :name="`permissions[${id}][can_view_all_tasks]`" :value="perm.can_view_all_tasks">
                </div>
            </template>
                ...
                <div class="flex gap-2 mb-8">
                    <template x-for="i in [1,2,3]" :key="i">
                        <div class="flex-1 h-1.5 rounded-full transition-colors duration-300"
                             :class="step >= i ? 'bg-indigo-600' : 'bg-slate-100'"></div>
                    </template>
                </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <input type="date" name="start_date" x-model="form.start_date" class="w-full border rounded-xl p-3">
        </div>
        <div>
            <label class="font-bold text-sm block mb-1">تاريخ الانتهاء</label>
            <input type="date" name="end_date" x-model="form.end_date" class="w-full border rounded-xl p-3">
        </div>
    </div>
</div>
                ...
                <div x-show="step===2">
                    ...
                    <input x-model="leaderSearch" placeholder="ابحث عن اسم..." class="w-full border rounded-xl p-3 mb-3 bg-slate-50">
                    <div class="border rounded-xl max-h-64 overflow-y-auto divide-y shadow-sm">
                        <template x-for="u in filteredLeaders" :key="u.id">
                            <div @click="selectLeader(u)" :class="leader?.id === u.id ? 'bg-indigo-50' : ''"
                                 class="px-4 py-3 hover:bg-slate-50 cursor-pointer flex justify-between items-center transition">
                ...
                <div x-show="step===3">
                    ...
                    <input x-model="memberSearch" placeholder="ابحث عن أعضاء..." class="w-full border rounded-xl p-3 mb-4 bg-slate-50">
                    
                    <div class="border rounded-xl max-h-40 overflow-y-auto mb-6 bg-white" x-show="memberSearch">
                        <template x-for="u in filteredMembers" :key="u.id">
                            <div @click="addMember(u)" class="px-4 py-2 hover:bg-indigo-50 cursor-pointer border-b last:border-0">
                                <span x-text="u.name"></span>
                            </div>
                        </template>
                    </div>
                        ...
                        <template x-for="m in members" :key="m.id">
                            <div class="border border-slate-200 rounded-2xl p-4 bg-slate-50/50">
                                <div class="flex justify-between items-center mb-3">
                                    ...
                                    <button type="button" @click="removeMember(m.id)" class="text-red-400 hover:text-red-600">✕</button>
                                </div>
                                <div class="flex gap-4">
                                    <label class="flex items-center gap-2 text-sm cursor-pointer select-none">
                                        <input type="checkbox" x-model="permissions[m.id].can_create_tasks" class="rounded text-indigo-600">
                                        إنشاء مهام
                                    </label>
                                    <label class="flex items-center gap-2 text-sm cursor-pointer select-none">
                                        <input type="checkbox" x-model="permissions[m.id].can_view_all_tasks" class="rounded text-indigo-600">
                                        رؤية الجميع
                                    </label>
                                </div>
                            </div>
            ...
            <div class="sticky bottom-0 left-0 right-0 p-6 bg-white border-t flex items-center justify-between gap-4">
    
    <div class="flex-none">
        <button type="button" 
                x-show="step > 1" 
                @click="step--" 
                class="px-6 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition">
            السابق
        </button>
    </div>
    ...
    <div class="flex-none">
        <button type="button" 
                x-show="step < 3" 
                @click="step++" 
                class="px-8 py-2.5 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition">
            التالي
        </button>

        <button type="submit" 
                x-show="step === 3" 
                class="px-8 py-2.5 bg-emerald-600 text-white rounded-xl font-bold shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition">
...
open:false,step:1,mode:'create',
users,missions,
form:{id:null,title:'',description:'',leader:null},
members:[],permissions:{},
leaderSearch:'',memberSearch:'',
leader:null,
openCreate(){
this.reset()
this.open=true
...
u.name.includes(this.memberSearch)&&
...
addMember(u){
this.members.push(u)
this.permissions[u.id]={can_create_tasks:true,can_view_all_tasks:false}
},
