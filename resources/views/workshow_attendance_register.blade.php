    ...
    <div class="max-w-4xl mx-auto p-4 md:p-8">
        <div class="text-center mb-10">
            <h1 class="text-3xl md:text-4xl font-extrabold text-blue-800">
                تسجيل الحضور في الورشة
            </h1>
            <p class="mt-2 text-lg text-gray-600">سجل حضورك للورشة بملئ النموذج أدناه</p>
        </div>
        ...
        @if (session('success') || session('warning') || session('error'))
            ...
            <div class="{{ $color }} border-r-4 p-4 rounded-lg mb-6 shadow-md" role="alert">
                <p class="font-bold">@if($type === 'success') نجاح @elseif($type === 'warning') تنبيه @else خطأ @endif</p>
                <p>{{ $message }}</p>
            </div>
        @endif
            ...
            <div class="bg-white p-6 rounded-xl shadow-lg border border-blue-100 mb-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-gray-600">
                    <div class="flex items-center space-x-2 space-x-reverse">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clip-rule="evenodd" /></svg>
                        <span>التاريخ: {{ \Carbon\Carbon::parse($workshop->created_at)->format('Y-m-d') }}</span>
                    </div>
                    <div class="flex items-center space-x-2 space-x-reverse">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l3 3a1 1 0 001.414-1.414L10 10V6z" clip-rule="evenodd" /></svg>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white p-6 rounded-xl shadow-lg">
                    ...
                    <form method="POST" action="{{ route('workshow_attendance_register') }}">
                        ...
                        <div class="mb-4">
                            <label for="job_title" class="block text-sm font-medium text-gray-700 mb-1">المسمى الوظيفي (اختياري)</label>
                        ...
                        <div class="mb-6">
                            <label for="department" class="block text-sm font-medium text-gray-700 mb-1">القسم/الدائرة/المديرية (اختياري)</label>
                ...
                <div class="bg-white p-6 rounded-xl shadow-lg overflow-x-auto">
                    <h2 class="text-2xl font-semibold text-gray-800 mb-4">قائمة الحضور ({{ $attendances->count() }})</h2>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 attendance-table">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">
                                        المسمى الوظيفي
                                    </th>
                                    <th class="px-3 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        وقت التسجيل
        ...
        @else
            ...
            <div class="bg-yellow-100 border-r-4 border-yellow-500 p-6 rounded-xl shadow-lg text-yellow-800 text-center text-xl font-medium">
