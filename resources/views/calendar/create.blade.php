...
<body class="bg-gray-100 dark:bg-[#060818] min-h-screen" x-data="eventForm()" :class="{ 'dark': $store.app.theme === 'dark' || $store.app.isDarkMode }">
    ...
    <main class="container mx-auto px-4 pb-20">
        <div class="max-w-4xl mx-auto">
            ...
            {{-- Main Form Card --}}
            <div class="bg-white dark:bg-[#1b2e4b] rounded-3xl shadow-2xl overflow-hidden border border-gray-100 dark:border-[#191e3a] relative">
                ...
                <form id="event_form" method="POST" 
                    ...
                    <div x-show="currentStep === 0" x-cloak x-transition>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="relative flex flex-col items-center text-center p-4 bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-[#0e1726] dark:to-[#1b2e4b] rounded-2xl border-2 cursor-pointer transition hover:shadow-lg h-full"
                                       :class="selectedUser == '{{ auth()->id() }}' ? 'border-emerald-600 ring-2 ring-emerald-100 dark:ring-emerald-900' : 'border-transparent dark:border-[#191e3a]'">
                                    <input type="radio" name="target_user_id" value="{{ auth()->id() }}" 
                                           x-model="selectedUser" class="sr-only">
                                    <div class="w-12 h-12 bg-emerald-600 rounded-full flex items-center justify-center text-white mb-3 shadow-md">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                    </div>
                                    <p class="font-bold text-emerald-900 dark:text-emerald-400 text-sm">تقويمي الشخصي</p>
                                    <svg x-show="selectedUser == '{{ auth()->id() }}'" class="w-6 h-6 text-emerald-600 dark:text-emerald-400 absolute top-2 left-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                                </label>

                                @foreach ($managedUsers as $u)
                                <label class="relative flex flex-col items-center text-center p-4 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-[#0e1726] dark:to-[#1b2e4b] rounded-2xl border-2 cursor-pointer transition hover:shadow-lg h-full"
                                       :class="selectedUser == '{{ $u->id }}' ? 'border-blue-600 ring-2 ring-blue-100 dark:ring-blue-900' : 'border-transparent dark:border-[#191e3a]'">
                                    <input type="radio" name="target_user_id" value="{{ $u->id }}" 
                                           x-model="selectedUser" class="sr-only"
                                           {{ old('target_user_id', $event->target_user_id ?? '') == $u->id ? 'checked' : '' }}>
                                    <div class="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-lg mb-3 shadow-md">
                                        {{ mb_substr($u->name, 0, 1) }}
                                    </div>
                                    <p class="font-bold text-blue-900 dark:text-blue-400 text-sm line-clamp-2 leading-tight">{{ $u->name }}</p>
                                    <svg x-show="selectedUser == '{{ $u->id }}'" class="w-6 h-6 text-blue-600 dark:text-blue-400 absolute top-2 left-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                                </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div x-show="currentStep === 1" x-cloak x-transition>
                        ...
                        <div class="space-y-6">
                            {{-- Title --}}
                            <div>
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">عنوان النشاط أو المهمة *</label>
                                <input type="text" name="title" value="{{ old('title', $event->title ?? '') }}"
                                    class="w-full border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-2xl px-5 py-4 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition shadow-inner dark:text-gray-300" required>
                                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">تصنيف النشاط *</label>
                                    <select name="type" class="w-full border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-2xl px-5 py-4 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition shadow-inner dark:text-gray-300" required>
                                ...
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">اسم البرنامج (اختياري)</label>
                                    <input type="text" name="program" value="{{ old('program', $event->program ?? '') }}" class="w-full border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-2xl px-5 py-4 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition shadow-inner dark:text-gray-300">
                    ...
                    <div x-show="currentStep === 2" x-cloak x-transition>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <label class="block text-sm font-bold text-emerald-800 dark:text-emerald-500 mb-2">بداية النشاط</label>
                                <div class="flex gap-2">
                                    <input type="date" name="start_date_day" value="{{ old('start_date_day', isset($event) ? $event->start_date->format('Y-m-d') : '') }}" class="flex-2 border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-xl px-4 py-3 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition w-full dark:text-gray-300 dark:color-scheme-dark" required>
                                    <input type="time" name="start_date_time" value="{{ old('start_date_time', isset($event) ? $event->start_date->format('H:i') : '08:00') }}" class="flex-1 border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-xl px-4 py-3 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition dark:text-gray-300 dark:color-scheme-dark" required>
                                </div>
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-red-800 dark:text-red-400 mb-2">نهاية النشاط</label>
                                <div class="flex gap-2">
                                    <input type="date" name="end_date_day" value="{{ old('end_date_day', isset($event) ? $event->end_date->format('Y-m-d') : '') }}" class="flex-2 border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-xl px-4 py-3 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition w-full dark:text-gray-300 dark:color-scheme-dark" required>
                                    <input type="time" name="end_date_time" value="{{ old('end_date_time', isset($event) ? $event->end_date->format('H:i') : '09:00') }}" class="flex-1 border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-xl px-4 py-3 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition dark:text-gray-300 dark:color-scheme-dark" required>
                                </div>
                            </div>
                        </div>
                    ...
                    <div x-show="currentStep === 3" x-cloak x-transition>
                        ...
                        <div x-data="{ selectedColor: '{{ old('bg_color', $event->bg_color ?? '#16a34a') }}' }" class="space-y-4 mb-6">
                            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300">لون التمييز</label>
                            ...
                            <div class="flex flex-wrap gap-4 p-1">
                                ...
                                <button type="button" @click="selectedColor = '{{ $color }}'"
                                        ...
                                        :class="selectedColor === '{{ $color }}' ? 'border-gray-800 dark:border-white scale-110' : 'border-white dark:border-[#191e3a] opacity-70 hover:opacity-100'">
                                </button>
                                @endforeach
                            </div>
                        </div>

                        <div class="flex items-center gap-4 bg-gray-50 dark:bg-[#0e1726] p-5 rounded-2xl border-2 border-gray-100 dark:border-[#191e3a] mb-6">
                            ...
                            <span class="font-bold text-gray-800 dark:text-gray-200">إظهار للجميع (نشاط عام)</span>
                        </div>

                        <div class="space-y-2">
                            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300">ملاحظات</label>
                            <textarea name="notes" rows="3" class="w-full border-2 border-gray-100 dark:border-[#191e3a] bg-gray-50 dark:bg-[#0e1726] rounded-2xl px-5 py-4 focus:bg-white dark:focus:bg-[#1b2e4b] focus:border-emerald-500 transition shadow-inner dark:text-gray-300">{{ old('notes', $event->notes ?? '') }}</textarea>
                        </div>
