<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-6 space-y-4">
        <p>
            هنا عليك أن تحدد النسبة المستهدفة في كل سنة (قد تكون ثابته لكل سنة <span class="bg-blue-100">5%</span>)
        </p>
        <p>
            سيقوم البرنامج بحساب المستهدف لكل سنة بدءً من سنة مابعد خط الأساس إلى سنة 2040
        </p>
        <p>
            هذه النسبة ستكون مطبقة لكل من:
            <li>المؤشر العام</li>
            <li>المؤشر للقطاعات</li>
        </p>
        <form method="POST" action="{{ route('indicator_target.update', $indicator) }}">
            ...
            @foreach ($targets as $index => $target)
                <div class="flex items-center gap-4 bg-white p-4 rounded-lg shadow border">
                    ...
                    <input type="number" step="0.01" name="targets[{{ $index }}][target_value]"
