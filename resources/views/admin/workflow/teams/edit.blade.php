<x-app-layout>
    ...
    <div class="py-6 max-w-3xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded p-6">
            <form action="{{ route('admin.workflow.teams.update', $team) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full border rounded px-3 py-2 @error('description') border-red-500 @enderror">{{ old('description', $team->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach($users as $user)
                            <label class="flex items-center gap-2 py-1">
                                <input type="checkbox" name="user_ids[]" value="{{ $user->id }}"
                                       {{ in_array($user->id, old('user_ids', $teamUserIds)) ? 'checked' : '' }}
