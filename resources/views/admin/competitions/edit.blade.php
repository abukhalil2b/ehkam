<x-app-layout>
    ...
    <div class="py-12" dir="rtl">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div class="mb-6 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg text-right"
                    role="alert">
            ...
            @if(session('error'))
                <div class="mb-6 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg text-right"
                    role="alert">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 ml-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-reverse space-x-3">
                            ...
                            <span class="text-sm text-gray-600">
                                الرمز: <span
                                    class="font-mono font-bold text-gray-900">{{ $competition->join_code }}</span>
                            </span>
                        </div>
                ...
                <form action="{{ route('admin.competitions.update', $competition) }}" method="POST"
                    ...
                    <div class="mb-6">
                        ...
                        <div class="relative">
                            <input type="text" id="title" name="title" required maxlength="255" x-model="title"
                            ...
                            <div class="absolute left-3 top-3 text-xs text-gray-400" x-text="`${titleLength}/255`">
                            </div>
                        </div>
                        @error('title')
                    ...
                    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        <div class="space-y-2 text-sm text-blue-800">
                            <div class="flex items-center">
                                <svg class="w-4 h-4 ml-2 text-blue-600" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if($competition->status !== 'closed')
                                <div class="flex items-start mt-3 pt-3 border-t border-blue-300">
                                    <svg class="w-4 h-4 ml-2 text-blue-600 mt-0.5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                    </svg>
                                    <span class="text-xs">ملاحظة: التعديل محدود لأن المسابقة
                                        {{ $competition->status === 'started' ? 'بدأت بالفعل' : 'انتهت' }}. يمكن تغيير
                                        العنوان فقط.</span>
                                </div>
                            @endif
                        </div>
            ...
            <!-- Clear Data Section (for testing) -->
            @if($competition->participants()->count() > 0)
                <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-right">
                    <h3 class="font-semibold text-yellow-900 mb-2 flex items-center">
                        <svg class="w-5 h-5 ml-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                    ...
                    <p class="text-sm text-yellow-800 mb-4">
                        هذه المسابقة تحتوي على <strong>{{ $competition->participants()->count() }}</strong> مشارك.
                        استخدم هذا الزر لحذف جميع المشاركين والإجابات وإعادة المسابقة لحالة "مغلقة".
                    </p>
                    <form action="{{ route('admin.competitions.clear-data', $competition) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                            onclick="return confirm('⚠️ تحذير: سيتم حذف جميع المشاركين ({{ $competition->participants()->count() }}) وإجاباتهم نهائياً. هل أنت متأكد؟')"
