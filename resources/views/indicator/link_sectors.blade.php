<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <p>
                هذه القطاعات سوف يتم تحديد مستهدف لها ونحتاج هنا لمرة واحدة فقط ربط هذه القطاعات بالمؤشر الذي سوف تقوم
                القطاعات بتغذيته بالمحقق

            </p>
            <p>
                بعد الربط عليك بتحديد
               <li> خط الأساس (القيمة)</li>
               <li> سنة الأساس</li>
            </p>
            <form action="{{ route('indicator.sectors.update', $indicator) }}" method="POST"
                ...
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach ($allSectors as $sector)
                            <label
                                class="relative flex items-center p-4 border rounded-xl cursor-pointer hover:bg-gray-50 transition-colors {{ in_array($sector->id, $currentSectorIds) ? 'border-indigo-500 bg-indigo-50/30' : 'border-gray-200' }}">
                ...
                <div class="px-8 py-4 bg-gray-50 flex justify-between items-center">
                    <a href="{{ route('indicator.show', $indicator) }}"
                        class="text-sm text-gray-500 hover:text-gray-700">إلغاء</a>
                    <button type="submit"
                        class="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all">
