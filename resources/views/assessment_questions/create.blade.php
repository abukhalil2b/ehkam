        ...
        <form method="POST" action="{{ route('assessment_questions.store') }}"
            ...
            <div class="mb-6">
                ...
                <select name="type" id="type"
                    ...
                    <option value="text" {{ old('type') == 'text' ? 'selected' : '' }}> (إجابة نصية مفتوحة)
                    </option>
                </select>
                @error('type')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-8" id="max_point_field">
                <label for="max_point" class="block text-sm font-medium text-gray-700 mb-2">الحد الأقصى للنقاط
                    (1-20):</label>
                <input type="range" name="max_point" id="max_point" min="1" max="20"
                    value="{{ old('max_point', 10) }}"
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-purple-500 focus:ring-purple-500 p-3"
                    oninput="document.getElementById('range_value').innerText = this.value">
                <span id="range_value" class="text-xl font-bold text-purple-600 w-8 text-center">10</span>
                @error('max_point')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
